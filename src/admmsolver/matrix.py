...
class DenseMatrix(MatrixBase):
    ...
    def __matmul__(self, other) -> Union['DenseMatrix', np.ndarray]:
        ...
        assert isinstance(other, MatrixBase) or (
            isinstance(other, np.ndarray) and other.ndim <= 2)
        if isinstance(other, np.ndarray):
            ...
            if isinstance(other, ScaledIdentityMatrix):
                ...
                res = np.zeros(
                    (self.shape[0], other.shape[1]), dtype=np.complex128)
    ...
    def __add__(self, other):
        return _add_matrices(self, other)
...
class ScaledIdentityMatrix(MatrixBase):
    ...
    def __init__(
        ...
        assert type(coeff) in [complex, float,
                               np.float64, np.complex128], type(coeff)
    ...
    def diagonals(self) -> np.ndarray:
        if not self.is_diagonal():
            raise RuntimeError(
                "Diagonals of a rectangular matrix is ill defined!")
...
class DiagonalMatrix(MatrixBase):
    ...
    def __init__(self, diagonals, shape: Optional[Tuple[int, int]] = None) -> None:
        ...
        if shape is None:
            ...
            self.shape = shape
        assert min(
            *self.shape) == diagonals.size, f"{self.shape} {diagonals.size}"
    ...
    def __matmul__(self, other: Union[MatrixBase, np.ndarray]) -> Union[MatrixBase, np.ndarray]:
        ...
        if isinstance(other, np.ndarray):
            ...
            if other.ndim == 1:
                ...
                res[0:min_len, ...] = \
                    np.einsum(
                        'd,d...->d...', self._diagonals[0:min_len], other[0:min_len, ...], optimize=True)
                return res
...
class PartialDiagonalMatrix(MatrixBase):
    ...
    def __init__(self, matrix: Union[np.ndarray, MatrixBase], rest_dims: tuple) -> None:
        ...
        self.ndim = 2
        self.shape = (matrix.shape[0]*np.prod(rest_dims),
                      matrix.shape[1]*np.prod(rest_dims))
...
def asmatrixtype(a) -> MatrixBase:
    assert isinstance(a, MatrixBase) or (
...
def _add_DiagonalMatrix_DenseMatrix(a, b):
    return DenseMatrix(b.asmatrix() + np.diag(a.diagonals))


def _add_DiagonalMatrix_DiagonalMatrix(a, b):
    return DiagonalMatrix(a.diagonals + b.diagonals)


def _add_DiagonalMatrix_PartialDiagonalMatrix(a, b):
    a_diag = a.diagonals.reshape(b.matrix.shape[0], -1)
    if np.allclose(a_diag, a_diag[:, 0:1]):
        return PartialDiagonalMatrix(
...
def _add_ScaledIdentityMatrix_ScaledIdentityMatrix(a, b):
    return ScaledIdentityMatrix(a.shape, a.coeff + b.coeff)


def _add_ScaledIdentityMatrix_DiagonalMatrix(a, b):
    return DiagonalMatrix(a.coeff*np.ones(a.shape[0]) + b.diagonals)


def _add_ScaledIdentityMatrix_PartialDiagonalMatrix(a, b):
    return PartialDiagonalMatrix(
        ScaledIdentityMatrix(
            b.matrix.shape[0], a.coeff) + b.matrix,
        b.rest_dims)


def _add_matrices(a, b):
    assert isinstance(a, MatrixBase)
    assert isinstance(b, MatrixBase)

    assert a.shape == b.shape

    func_name_1 = f"_add_{type(a).__name__}_{type(b).__name__}"

    add_func1 = globals().get(func_name_1)
    if add_func1 is not None:
        return add_func1(a, b)

    func_name_2 = f"_add_{type(b).__name__}_{type(a).__name__}"
    add_func2 = globals().get(func_name_2)
    if add_func2 is not None:
        return add_func2(b, a)
